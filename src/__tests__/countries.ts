...
describe('Test Countries', () => {
  it('Request /countries?filter=AT&order=asc', async () => {
    const result = await request(app)
      .get('/countries?filter=AT&order=asc')
      .send();
  ...
  it('Request /countries?filter=B&order=asc', async () => {
    const result = await request(app)
      .get('/countries?filter=B&order=asc')
      .send();

    expect(result.status).toBe(200);
    expect((result.body as Country[]).map(({ vat }) => vat))
      .toStrictEqual([20, 20, 21]);
  });

  it('Request /countries?filter=B&order=desc', async () => {
    const result = await request(app)
      .get('/countries?filter=B&order=desc')
      .send();

    expect(result.status).toBe(200);
    expect((result.body as Country[]).map(({ vat }) => vat))
      .toStrictEqual([21, 20, 20]);
  });
  it('Request /countries?order=asc', async () => {
    const result = await request(app)
      .get('/countries?filter=&order=asc')
      .send();
  ...
  it('Request /countries?filter=B&order=bad', async () => {
    const result = await request(app)
      .get('/countries?filter=B&order=bad')
      .send();
